...
class EventVolunteerList extends React.Component {
  ...
  sendApprovalEmail = (v_email, o_email, topic, text) => {
    axios
      .post(
        `/send-email?recipient=${v_email}&sender=${o_email}&topic=${topic}&text=${text}`
      )
    ...
    const mappedPendingVolunteers = this.props.pendingVolunteers.map(el => {
      ...
      return (
        <div key={el.v_id}>
          <p
            id="pending-vol-name"
            onClick={() => this.getVolunteerInfo(el.v_id)}
          >
            {el.v_name}
          </p>
          <button
          ...
          <div className="pending">
            ...
            {this.state.showVolInfo ? (
              <div key={selectedVolunteerInfo.v_id} className="hidden-vol-info">
                <p>
                  Name:<span>{selectedVolunteerInfo.v_name}</span>
                </p>
                <p>
                  Location:<span> {selectedVolunteerInfo.v_location}</span>
                </p>
                <p>
                  Email:<span>{selectedVolunteerInfo.v_email}</span>
                </p>
                <img
                  src={selectedVolunteerInfo.v_image}
                  alt={selectedVolunteerInfo.v_name}
                />
