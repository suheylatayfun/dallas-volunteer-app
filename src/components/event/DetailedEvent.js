...
class DetailedEvent extends React.Component {
  ...
  volunteerEvent=()=>{
    ...
    this.props.volunteerEvent(v_id,this.props.e_id).then().catch(()=>window.alert("You have already volunteered for this event."))
  }
  render() {
    ...
    const mappedEvent = this.props.event.length > 0 ? this.props.event.map((el, i) => {
      return (
        <div key={el.e_id} className="detailed-event">
            <TiDeleteOutline  onClick={this.props.toggleEvent||this.props.openEventInfo} id="delete" size={30}/>
          <h3>{el.e_title}</h3>
          
          {this.props.loading ? <img src='https://resources.humandx.org/static/img/loading_spinner.gif' alt='Loading..' /> : <img src={el.e_image} alt="event" id="detailed-ev-image"/>}
          
          {this.props.role === 'organization'? null : (
            <Link to="/volunteer/profile"><button id="join-button" onClick={this.volunteerEvent}>VOLUNTEER</button></Link> 
            )  } 
